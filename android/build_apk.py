...
def build_resources(tmp, version_code, version_name):
    ...
    rjava = os.path.join(tmp, "rjava")
    os.makedirs(rjava, exist_ok=True)
...
def build_dex(tmp):
    ...
    srcs = sorted(_glob.glob(os.path.join(root, "**", "*.java"), recursive=True))
    # R.java is generated by aapt2 link (--java) in the build temp dir
    srcs += sorted(_glob.glob(os.path.join(tmp, "rjava", "**", "*.java"), recursive=True))
    if not srcs:
        sys.exit("[build] no .java sources found under " + root)
